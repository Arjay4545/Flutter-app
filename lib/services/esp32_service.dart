...
class ESP32Service {
  ...
  void startListening() {
    ...
    _currentDataSubscription = _database
      ...
      if (event.snapshot.exists) {
        try {
          ...
          _updateStatus('Received data from ESP32: ${data['deviceId'] ?? 'Unknown'}');
        } catch (e) {
    ...
    _devicesSubscription = _database
      ...
      if (event.snapshot.exists) {
        try {
          final devicesMap = Map<String, dynamic>.from(event.snapshot.value as Map);
  ...
  Future<List<Map<String, dynamic>>> getHistoricalData({
    ...
    try {
      ...
      if (startDate != null) {
        query = query.orderByKey().startAt(startDate.millisecondsSinceEpoch.toString());
      }
      
      if (endDate != null) {
        query = query.orderByKey().endAt(endDate.millisecondsSinceEpoch.toString());
      }
      ...
      if (snapshot.exists) {
        ...
        historyMap.forEach((timestamp, data) {
          ...
          entry['date'] = DateTime.fromMillisecondsSinceEpoch(int.parse(timestamp) * 1000);
  ...
  Map<String, dynamic> getDeviceStatistics() {
    final onlineDevices = _connectedDevices.where((d) => d['isOnline'] == true).length;
